...
import { checkout } from '../features/product/orderSlice'; // Import the checkout action
...
const Checkout = () => {
  ...
  const handleSubmitOrder = () => {
    const data = {  ...orderDetails,  cartItems,  totalPrice: totalPrice + deliveryCharges,  totalQuantity,  totalDiscount,  };

    console.log("Data being submitted:", data); // Log the data for debugging

    // Dispatch the checkout action with the order data
    dispatch(checkout(data)).then((action) => {
      if (action.type === 'order/checkout/fulfilled') {
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex items-center space-x-2 text-gray-500">
          <Link to="/account" className="text-gray-600">Account</Link>  <span>/</span>
          <Link to="/my-account" className="text-gray-600">My Account</Link>  <span>/</span>
          <Link to="/shop" className="text-gray-600">Product</Link>  <span>/</span>
          <Link to="/cart" className="text-gray-600">View Cart</Link>  <span>/</span>
      ...
      <div className="container mx-auto px-4 flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/3 bg-white shadow-md rounded-lg p-6">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleSubmitOrder} className="w-full bg-red-500 text-white py-2 rounded-md hover:bg-red-600">  Place Order  </button>
          </div>
        </div>
      </div>
    </div>
