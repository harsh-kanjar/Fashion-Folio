...
const Header = () => {
  ...
  const handleLogout = () => {
    localStorage.removeItem('ecom-auth-token');
    navigate("/signup");
  };

  const isLoggedIn = !!localStorage.getItem('ecom-auth-token');

  return (
    <header className="bg-white  text-gray-900   py-4 px-4">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <nav className="hidden md:block">
          <ul className="flex space-x-4">
            ...
            {
              localStorage.getItem('ecom-auth-token') ?
                <li className='cursor-pointer px-2 py-1 hover:shadow-md hover:bg-red-500 hover:text-white transition duration-300 ease-in-out transform rounded'>
